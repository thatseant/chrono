...
exports.strictOption = function () {
  ...
  return exports.mergeOptions([exports.en(strictConfig), // exports.de(strictConfig),
...
exports.en.casual = function (config) {
  ...
  option.parsers.unshift(new parser.ENCasualDateParser()); // option.parsers.unshift(new parser.ENCasualTimeParser());

  option.parsers.unshift(new parser.ENWeekdayParser()); // option.parsers.unshift(new parser.ENRelativeDateFormatParser());
...
var FIRST_REG_PATTERN = new RegExp("(^|\\s|T)" + "(?:(?:at|from|Time\:|Timing\:|When\:|🕔|\\()\\s*)??" + "(\\d{1,4}|noon|midnight)" + "(?:" + "(?:\\.|\\:|\\：)(\\d{1,2})" + "(?:" + "(?:\\:|\\：)(\\d{2})(?:\\.(\\d{1,6}))?" + ")?" + ")?" + "(?:\\s*(A\\.M\\.|P\\.M\\.|AM?|PM?|O\\W*CLOCK))?" + "(?=\\W|$)", 'i');
...
var PATTERN = /(\W|^)(now|tonight|last\s*night|(?:tomorrow|tmr|yesterday)\s*|tomorrow|tmr|yesterday)(?=\W|$)/i;
